@inject('SalesPayment', 'App\Http\Controllers\SalesPaymentController')
...
<h3 class="page-title">
    <b>Daftar Pembayaran</b> <small>Mengelola Pembayaran</small>
</h3>
<br/>
<div id="accordion">
    <form  method="post" action="{{route('filter-sales-payment')}}" enctype="multipart/form-data">
    @csrf
            ...
            <table id="example" style="width:100%" class="table table-striped table-bordered table-hover table-full-width">
                <thead>
                    <tr>
                        ...
                        <th width="10%" style='text-align:center'>No Pembayaran</th>
                        ...
                        <th width="10%" style='text-align:center'>Pembayaran dari Rekening</th>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{$item['sales_payment_no']}}</td>
                        <td>{{$item['sales_no']}}</td>
                        <td>{{$item['seller_name']}}</td>
                        <td>{{$item['buyer_name']}}</td>
                        <td>{{$item['payment_name']}}</td>
                        <td>{{$item['created_at']}}</td>
                        <td>{{$item['sales_payment_from']}}</td>
                        <td style='text-align:right'>{{number_format($item['sales_payment_amount'])}}</td>
                        <td class="">
                            <a type="button" class="btn btn-outline-warning btn-sm" href="{{ url('/sales-payment/detail/'.$item['sales_payment_id']) }}">Detail</a>
                        </td>
                    </tr>
                    <?php $no++; ?>
                    @endforeach
